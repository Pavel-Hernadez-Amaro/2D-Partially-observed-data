...
imse_beta_2d=function (beta_hat, beta_true, R0, domain_area) 
{
  imse <- as.numeric((t(beta_hat) %*% R0 %*% beta_hat - 2 * 
                        t(beta_hat) %*% R0 %*% beta_true + t(beta_true) %*% 
                        R0 %*% beta_true)/domain_area)
  return(imse)
}
