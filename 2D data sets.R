...
library(caret)
library(ks)
# orig <- load.image("C:/Users/user/Desktop/Trabajo/Escuela/Doctorado/Pavel/Tesis/2do paper/codigo/Paper 2 Classification/2D/2D-Partially-observed-data/Data sets/archive/Air Pollution Image Dataset/Air Pollution Image Dataset/Country_wise_Dataset/India/Dimapur/b_Moderate/Dimapur_MOD_2023-02-10-08.30-1.jpg")  # Read your RGB image
# gray_image <- grayscale(orig, method = "Luma")  # Convert to grayscale
# plot(orig)  # Display the grayscale image

# data=read.csv("C:/Users/user/Desktop/Trabajo/Escuela/Doctorado/Pavel/Tesis/2do paper/codigo/Paper 2 Classification/2D/2D-Partially-observed-data/Data sets/archive/Dataset_for_AQI_Classification/Dataset_for_AQI_Classification/train_data.csv")
# length(data$AQI_Class)
# table(data$AQI_Class)
...
# data$id=rep(0,N)
# 
# # data_Dimapur=read.csv("C:/Users/user/Desktop/Trabajo/Escuela/Doctorado/Pavel/Tesis/2do paper/codigo/Paper 2 Classification/2D/2D-Partially-observed-data/Data sets/archive/Air Pollution Image Dataset/Air Pollution Image Dataset/Country_wise_Dataset/India/Dimapur/Dimapur_AQI_All_Info.csv")
# 
# dir_mod="C:/Users/user/Desktop/Trabajo/Escuela/Doctorado/Pavel/Tesis/2do paper/codigo/Paper 2 Classification/2D/2D-Partially-observed-data/Dimapur/b_Moderate"
# dir_un="C:/Users/user/Desktop/Trabajo/Escuela/Doctorado/Pavel/Tesis/2do paper/codigo/Paper 2 Classification/2D/2D-Partially-observed-data/Dimapur/c" 

####c_Unhealthy_for_Sensitive_Groups"

# THIS WAS FOR CHANCHING THE NAME OF THE JPG FILES
# DO NOT RUN AGAIN

# for (i in 1:N) {
# 
#   char_vector <- unlist(strsplit(data$Filename[i], ""))
#   
#   aux_mod=which(list.files(dir_mod)==paste(char_vector,collapse = ""))
#   aux_un=which(list.files(dir_un)==paste(char_vector,collapse = ""))
#   
#   if (char_vector[9]=="M") {
# 
#     aux=list.files(dir_mod)[aux_mod]
#     folder <- dir_mod
#     file <- list.files(folder, pattern = "*.jpg", full.names = TRUE)[aux_mod]
#     
#     id=paste(char_vector[c(9:12,30:(length(char_vector)-4))],collapse = "")
#     
#     new_name=paste0(paste0(folder,"/"),id)
...
#   }else{
#     
#     aux=list.files(dir_un)[aux_un]
#     folder <- dir_un
#     file <- list.files(folder, pattern = "*.jpg", full.names = TRUE)[aux_un]
#     
#     id=paste(char_vector[c(9:14,33:(length(char_vector)-4))],collapse = "")
#     
#     new_name=paste0(paste0(folder,"/"),id)
#     new_name=paste0(new_name,".jpg")
#     
...
miss_error=miss_error_naive=response=response_naive=rep(0,N)
...
for (i in 1:N) {
  ...
  if (res$fit$fitted.values[i]<0.5) {
  ...
  miss_error_naive[i]  = as.double(response_naive[i]!=data_pollution$y[i])
}

sum(miss_error)
sum(miss_error_naive)
